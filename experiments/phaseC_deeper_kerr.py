"""
Experiment 15: Deeper Kerr stack (5 and 6 ODE layers)

Currently: 1 PerBandLinear + 3 Kerr-ODE = 4 layers total.
Each local coupling step propagates info ~4 bands.
3 Kerr layers = effective reach ~12 bands.

Test: 5 and 6 Kerr layers (6 and 7 total).
5 Kerr = ~20 band reach. 6 Kerr = ~24 band reach.
More sequential local steps = how real wave systems propagate long-range.

Also test Maestro-Add + 5 Kerr layers to see if the best interventions combine.
"""
...
def main():
    print("="*70)
    print("  Experiment 15: Deeper Kerr stack")
    ...
    configs = [
        ...
        ("Kerr 4L (1+3)",       dict(n_layers=4, ffn_type="kerr")),
        ("Kerr 6L (1+5)",       dict(n_layers=6, ffn_type="kerr")),
        ("Kerr 7L (1+6)",       dict(n_layers=7, ffn_type="kerr")),
        ("Maestro 4L (1+3)",    dict(n_layers=4, ffn_type="kerr", use_maestro=True)),
        ("Maestro 6L (1+5)",    dict(n_layers=6, ffn_type="kerr", use_maestro=True)),
    ]
    ...
    print(f"\n  Reference: 5-band + curriculum = +3.13%, Maestro 4L = +3.09%")
    print("="*70)
