"""
Experiment 11b: 128-band at 2000 iters (fair comparison)

Experiment 11 showed MLP overfitting at 4000 iters (val 1.54 -> 2.13).
This runs 2000 iters for honest locality penalty measurement.
Also adds weight decay to prevent overfitting at large model sizes.
"""
...
def train(mode, dataset, label=""):
    ...
    for i in range(MAX_ITERS):
        if i % EVAL_INTERVAL == 0 or i == MAX_ITERS-1:
            ...
            print(f"    step {i:>5} | train {losses['train']:.4f} | val {losses['val']:.4f}")
...
def main():
    print("="*70)
    print("  Experiment 11b: 128 bands, 2000 iters, weight decay 0.1")
    ...
    print("  --- MLP 128b ---")
    mlp_val = train("mlp", dataset, label="MLP 128b")
    print(f"\n  --- Kerr 128b ---")
    kerr_val = train("kerr", dataset, label="Kerr 128b")

    gap = (kerr_val / mlp_val - 1) * 100

    print(f"\n{'='*70}")
    print(f"  RESULTS (128 bands, 2000 iters, wd=0.1)")
    print(f"{'='*70}")
    print(f"  MLP 128b:   {mlp_val:.4f}  ({3176192:,} params)")
    print(f"  Kerr 128b:  {kerr_val:.4f}  ({1338630:,} params)  gap = {gap:+.2f}%")
    print(f"\n  Locality penalty scaling (all flat, 2000 iters):")
    print(f"    8b:   1.4%")
    print(f"    16b:  2.3%")
    print(f"    32b:  3.1%")
    print(f"    48b:  4.9%")
    ...
    print(f"    96b:  5.4%")
    print(f"    128b: {gap:.2f}%")
    print(f"\n  Note: Exp 11 (4000 iters, no wd) showed MLP overfitting")
    print(f"  (val 1.54 -> 2.13). Kerr was stable (1.56 -> 1.58).")
    print(f"  Kerr's lower param count = implicit regularization.")
    print("="*70)
