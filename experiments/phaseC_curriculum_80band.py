"""
Experiment 12: Curriculum crossover at 80 bands

Have: 32b (flat wins), 48b (flat wins), 64b (curriculum wins), 96b (curriculum wins)
Need: 80b to narrow the crossover point.
"""
...
PROG_STAGES_80 = [(0, 10), (667, 30), (1334, 80)]
...
def estimate_loss(model, dataset):
    ...
    model.train(); model.n_bands_active = old
    return out

def train(mode, dataset, curriculum=False, label=""):
    ...
    for i in range(MAX_ITERS):
        if curriculum and mode != "mlp":
            for step_thresh, nb in PROG_STAGES_80:
...
def main():
    print("="*70)
    print("  Experiment 12: Curriculum crossover at 80 bands")
    ...
    print("  --- MLP 80b flat ---")
    mlp_val = train("mlp", dataset, label="MLP flat")
    print(f"\n  --- Kerr 80b flat ---")
    kerr_flat = train("kerr", dataset, curriculum=False, label="Kerr flat")
    print(f"\n  --- Kerr 80b curriculum ---")
    kerr_curr = train("kerr", dataset, curriculum=True, label="Kerr curriculum")

    gap_flat = (kerr_flat / mlp_val - 1) * 100
    gap_curr = (kerr_curr / mlp_val - 1) * 100
    effect = gap_flat - gap_curr

    print(f"\n{'='*70}")
    print(f"  RESULTS (80 bands)")
    print(f"{'='*70}")
    print(f"  MLP flat:          {mlp_val:.4f}")
    print(f"  Kerr flat:         {kerr_flat:.4f}  gap = {gap_flat:+.2f}%")
    print(f"  Kerr curriculum:   {kerr_curr:.4f}  gap = {gap_curr:+.2f}%")
    print(f"\n  Curriculum effect: {effect:+.2f}pp")
    print(f"\n  Crossover table:")
    print(f"    32b: flat 3.1%, curr 6.3% -> flat wins (-3.2pp)")
    print(f"    48b: flat 4.9%, curr 8.2% -> flat wins (-3.3pp)")
    print(f"    64b: flat 4.88%, curr 3.13% -> curr wins (+1.75pp)")
    print(f"    80b: flat {gap_flat:.1f}%, curr {gap_curr:.1f}% -> {'curr wins' if effect > 0 else 'flat wins'} ({effect:+.1f}pp)")
    print(f"    96b: flat 5.4%, curr 4.6% -> curr wins (+0.8pp)")
    print("="*70)
