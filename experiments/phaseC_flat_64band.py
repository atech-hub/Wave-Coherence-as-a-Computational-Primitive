"""
Experiment 9: Flat 64-band Kerr-ODE

Does the 95.2% Phase B ceiling move without curriculum?
Single comparison: MLP vs flat Kerr at 64 bands.

Phase B (curriculum): Kerr gap = 3.42% (1.7511 vs 1.6932)
Flat trend: 3.1% at 32, 4.9% at 48, 5.4% at 96

If flat 64 < 3.42%: curriculum was hurting at 64 too, ceiling moves up
If flat 64 > 3.42%: curriculum earns its keep at 64 bands (crossover point)
"""
...
def main():
    print("="*70)
    print("  Experiment 9: Flat 64-band Kerr-ODE")
    ...
    print("  --- MLP 64b flat ---")
    mlp_val = train("mlp", dataset)
    print(f"\n  --- Kerr 64b flat ---")
    kerr_val = train("kerr", dataset)

    gap = (kerr_val / mlp_val - 1) * 100
    print(f"\n{'='*70}")
    print(f"  RESULTS")
    print(f"{'='*70}")
    print(f"  MLP 64b flat:  {mlp_val:.4f}")
    print(f"  Kerr 64b flat: {kerr_val:.4f}")
    print(f"  Flat gap:      {gap:+.2f}%")
    print(f"\n  Phase B (curriculum): 1.7511 vs 1.6932 = +3.42%")
    print(f"  Flat trend:    32b=3.1% | 48b=4.9% | 96b=5.4%")
    if gap < 3.42:
        print(f"\n  VERDICT: Flat wins -- curriculum was hurting at 64 bands too.")
        print(f"  The published 95.2% ceiling understates the architecture.")
    else:
        print(f"\n  VERDICT: Curriculum helps at 64 bands -- crossover is below 64.")
    print("="*70)
