...
**Important: novel architecture.** The Kerr-ODE is NOT a standard transformer. Trained models do not work with LM Studio, Ollama, llama.cpp, or Hugging Face Transformers natively — these clients have no code path for ODE integration or stencil coupling. However, the [Wave Server](https://github.com/atech-hub/wave-server) provides an OpenAI-compatible API that any chat UI can connect to. Train models with the [Wave Engine](https://github.com/atech-hub/wave-engine), serve them with the Wave Server, chat through any standard interface. GGUF export and HF model class integration are documented in [ENGINE-PATTERNS.md](ENGINE-PATTERNS.md) (Pattern 68) as prior art.
...
**Training engine:** The [Wave Engine](https://github.com/atech-hub/wave-engine) (Rust, Apache 2.0) is the production training engine for this architecture. Three compute tiers in a single binary: CPU (RK4-16 ODE), wgpu GPU (fused/perturbative ODE via WGSL shaders on any GPU vendor), and Candle CUDA (perturbative ODE with autograd). 24-layer production config, BPE tokenization, block-diagonal output projection, KV-cache inference, WCHK self-describing checkpoints. Successor to the [Kerr Engine](https://github.com/atech-hub/kerr-engine) (now parked — use wave-engine for all new work).

**Inference server:** The [Wave Server](https://github.com/atech-hub/wave-server) (Rust, Apache 2.0) serves trained models via an OpenAI-compatible API. Self-contained — no engine dependency. Any chat UI that speaks the OpenAI protocol (LM Studio, Open WebUI, SillyTavern, continue.dev) connects without modification. KV-cache, SSE streaming, bearer token auth, wave memory support. Successor to the [Kerr Server](https://github.com/atech-hub/kerr-server) (now parked — use wave-server for all new work).
...
**[Wave Structure Emergence](investigations/wave-structure/INVESTIGATION.md) (active)** — Does a trained wave-engine model actually build the harmonic phase structure the theory predicts? First diagnostic data: phase clustering at 0.553 (structured, not random), semantic discrimination at 1.0x (not yet emerged), different token pairs mapping to different harmonics (n=1 for boy/ball, n=5 for cat/dog, n=6 for sat/kicked). Three architectural findings: (1) single-grid harmonic embeddings degenerate at high vocab/bands ratios — 50K vocab at 84 bands gives 0.0016 adjacent separation, training NaNs; (2) multi-grid coprime embeddings (Pattern 53) give 11,800x improvement — resolves any vocabulary at any dimension; (3) per-band ODE input magnitude clamping prevents phase wrapping when maestro corrections grow. Combined: zero NaN in 3000 iterations at 168-dim with 2K BPE. Tracking six diagnostics across model sizes and training passes. Built into the engine as `--analyze` flag using `cos(n × Δθ)` harmonic coherence.
