...
| `experiments/` | 30 training experiments (Phases 1-22d + Phase A + Phase B) with results, plus PyTorch cross-framework verification -- spectral persistence, progressive learning, harmonic decoding, wave packet engine, weight spectral analysis, harmonic attention, LC circuit layer, Kerr-ODE layer, per-band Kerr, inverse Kerr reversibility analysis, analytical L0 replacement, wider clamp analysis, RK4 integration, full stack integration test, and 7-variant integration sweep |
...
**Phase B: two-stage magnitude training improves the stack further.** A 7-variant controlled sweep tested three integration points from the spherical coherence investigation: (1) trainable per-token magnitude, (2) band-aware routing (L0→low bands, L1-3→high bands), and (3) two-stage training (magnitude frozen during phase learning, freed after). In same-run comparison: **two-stage achieves 95.2% of MLP at 43.1% parameters**, improving the frozen baseline by 1.91%. Band routing hurts by 7-9% — layers need full-spectrum access for impedance matching. The magnitude CV tells the story: two-stage produces 2.46% CV (surgical optimisation on a stable phase foundation) versus 6.92% CV with magnitude always free (chasing a moving target). Less variation, better performance. Constrained freedom at the right time beats unconstrained freedom — the coupling principle (phase first, magnitude second) expressed as a training schedule.
